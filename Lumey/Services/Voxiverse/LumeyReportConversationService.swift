//
//  LumeyReportConversationService.swift
//  Lumey
//
...
@MainActor
final class LumeyReportConversationService: ObservableObject {
    @Published private(set) var snapshot: LumeyReportConversationSnapshot?
    @Published private(set) var isLoading = false
    @Published private(set) var isSending = false
    @Published private(set) var localOutgoingMessages: [LumeyReportConversationMessage] = []
    ...
    private struct PendingOutgoingPayload: Codable {
        let text: String
        let attachments: [LumeyConversationAttachment]
        let createdAt: Date
        let conversationID: String
    }

    init(container: CKContainer = CKContainer(identifier: LumeyReportConversationCloudKitSchema.containerIdentifier)) {
    ...
    func accept(report: SubmittedReport, modelContext: ModelContext) async {
        ...
        do {
            ...
            let now = Date()
            root[LumeyReportConversationCloudKitSchema.ConversationField.invitationState] = LumeyReportConversationState.accepted.rawValue as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.acceptedAt] = now as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.updatedAt] = now as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.reporterUnreadCount] = 0 as CKRecordValue
    ...
    func decline(report: SubmittedReport, modelContext: ModelContext) async {
        ...
        do {
            ...
            let root = try await fetchRecord(recordID, in: database)
            let now = Date()
            root[LumeyReportConversationCloudKitSchema.ConversationField.invitationState] = LumeyReportConversationState.declined.rawValue as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.declinedAt] = now as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.updatedAt] = now as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.reporterUnreadCount] = 0 as CKRecordValue
            _ = try await database.modifyRecords(saving: [root], deleting: [], savePolicy: .changedKeys, atomically: true)
    ...
    func sendReporterMessage(
        _ rawText: String,
        attachments: [LumeyConversationAttachment] = [],
        ...
        localOutgoingMessages.append(LumeyReportConversationMessage(
    ...
    var displayedMessages: [LumeyReportConversationMessage] {
    ...
    private func persistReporterMessage(messageID: String, payload: PendingOutgoingPayload, report: SubmittedReport, modelContext: ModelContext) async throws {
            ...
            let database = conversationDatabase(for: recordID)
            let root = try await fetchRecord(recordID, in: database)
            guard LumeyReportConversationState(rawValue: root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.invitationState)) == .accepted else {
                throw ConversationError.notAccepted
            }
            guard (root[LumeyReportConversationCloudKitSchema.ConversationField.acceptsReplies] as? NSNumber)?.boolValue ?? true else {
            ...
            let message = CKRecord(recordType: LumeyReportConversationCloudKitSchema.RecordType.message, recordID: messageRecordID)
            message.parent = CKRecord.Reference(recordID: root.recordID, action: .none)
            message[LumeyReportConversationCloudKitSchema.MessageField.messageID] = messageID as CKRecordValue
            message[LumeyReportConversationCloudKitSchema.MessageField.conversation] = CKRecord.Reference(recordID: root.recordID, action: .none)
            message[LumeyReportConversationCloudKitSchema.MessageField.conversationRecordName] = root.recordID.recordName as CKRecordValue
            message[LumeyReportConversationCloudKitSchema.MessageField.senderRole] = LumeyReportConversationSenderRole.reporter.rawValue as CKRecordValue
            message[LumeyReportConversationCloudKitSchema.MessageField.body] = payload.text as CKRecordValue
            message[LumeyReportConversationCloudKitSchema.MessageField.createdAt] = now as CKRecordValue
            message[LumeyReportConversationCloudKitSchema.MessageField.clientMessageID] = messageID as CKRecordValue
            ...
            var names = root[LumeyReportConversationCloudKitSchema.ConversationField.messageRecordNames] as? [String] ?? []
            if !names.contains(messageRecordID.recordName) {
                names.append(messageRecordID.recordName)
            }
            root[LumeyReportConversationCloudKitSchema.ConversationField.messageRecordNames] = names as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.lastMessageAt] = now as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.lastMessageSenderRole] = LumeyReportConversationSenderRole.reporter.rawValue as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.updatedAt] = now as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.staffUnreadCount] = (root.lumeyInt(LumeyReportConversationCloudKitSchema.ConversationField.staffUnreadCount) + 1) as CKRecordValue
    ...
    private func setDeliveryState(_ state: LumeyReportConversationDeliveryState, for messageID: String) {
    ...
    private func reconcileLocalMessages(with snapshot: LumeyReportConversationSnapshot) {
    ...
    private var outboxDirectory: URL {
        ...
        let directory = base.appendingPathComponent("LumeyReportConversationOutbox", isDirectory: true)
    ...
    private func writeAttachments(_ attachments: [LumeyConversationAttachment], to record: CKRecord) throws -> [URL] {
        var urls: [URL] = []
        do {
            for (offset, attachment) in attachments.prefix(3).enumerated() {
                ...
                urls.append(url)
                record[LumeyReportConversationCloudKitSchema.MessageField.attachment(index)] = CKAsset(fileURL: url)
                record[LumeyReportConversationCloudKitSchema.MessageField.attachmentName(index)] = attachment.name as CKRecordValue
                record[LumeyReportConversationCloudKitSchema.MessageField.attachmentType(index)] = attachment.typeIdentifier as CKRecordValue
            }
            record[LumeyReportConversationCloudKitSchema.MessageField.attachmentCount] = urls.count as CKRecordValue
            return urls
        } catch {
    ...
    private func readAttachments(from record: CKRecord) -> [LumeyConversationAttachment] {
        (0..<min(3, record.lumeyInt(LumeyReportConversationCloudKitSchema.MessageField.attachmentCount))).compactMap { offset in
            let index = offset + 1
            guard let url = (record[LumeyReportConversationCloudKitSchema.MessageField.attachment(index)] as? CKAsset)?.fileURL,
                  let data = try? Data(contentsOf: url) else { return nil }
            return LumeyConversationAttachment(
                name: record.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.attachmentName(index), fallback: "Attachment \(index)"),
                typeIdentifier: record.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.attachmentType(index), fallback: "public.data"),
                data: data
            )
        }
    }

    func fetchSummary(for report: SubmittedReport, modelContext: ModelContext) async -> LumeyReportConversationSnapshot {
        do {
    ...
    private func currentSnapshot(for report: SubmittedReport, modelContext: ModelContext) async throws -> LumeyReportConversationSnapshot {
        if let snapshot {
            return snapshot
        }
        return try await fetchSnapshot(for: report, modelContext: modelContext)
    ...
    private func fetchSnapshot(
        ...
        modelContext: ModelContext,
        markRead: Bool = false
    ) async throws -> LumeyReportConversationSnapshot {
        ...
        if markRead {
            let now = Date()
            root[LumeyReportConversationCloudKitSchema.ConversationField.reporterUnreadCount] = 0 as CKRecordValue
            root[LumeyReportConversationCloudKitSchema.ConversationField.reporterLastReadAt] = now as CKRecordValue
            _ = try await database.modifyRecords(saving: [root], deleting: [], savePolicy: .changedKeys, atomically: true)
        }
        ...
        if markRead {
            await LumeyReportConversationNotificationManager.clearReadConversationNotification(reportID: report.reportID)
        }
        try await LumeyReportConversationNotificationManager.registerSharedConversationNotifications(
            database: database,
    ...
    private func updatePublicReportConversationState(
        for report: SubmittedReport,
        state: LumeyReportConversationState,
        timestamp: Date
    ) async throws {
        let record = try await fetchPublicReport(for: report)
        record[LumeyReportConversationCloudKitSchema.PublicReportField.conversationState] = state.rawValue as CKRecordValue
        record[LumeyReportConversationCloudKitSchema.PublicReportField.conversationUpdatedAt] = timestamp as CKRecordValue
    ...
    private func updatePublicReportConversationLastActivity(
        ...
        record[LumeyReportConversationCloudKitSchema.PublicReportField.conversationState] = state.rawValue as CKRecordValue
        record[LumeyReportConversationCloudKitSchema.PublicReportField.conversationUpdatedAt] = timestamp as CKRecordValue
        record[LumeyReportConversationCloudKitSchema.PublicReportField.conversationLastMessageAt] = timestamp as CKRecordValue
        _ = try await container.publicCloudDatabase.modifyRecords(saving: [record], deleting: [], savePolicy: .changedKeys, atomically: true)
    }

    private func applyConversationPointer(from record: CKRecord, to report: SubmittedReport, modelContext: ModelContext) {
        report.conversationRecordName = record.lumeyString(LumeyReportConversationCloudKitSchema.PublicReportField.conversationRecordName)
        report.conversationZoneName = record.lumeyString(LumeyReportConversationCloudKitSchema.PublicReportField.conversationZoneName)
        report.conversationZoneOwnerName = record.lumeyString(LumeyReportConversationCloudKitSchema.PublicReportField.conversationZoneOwnerName)
        report.conversationShareURL = record.lumeyString(LumeyReportConversationCloudKitSchema.PublicReportField.conversationShareURL)
        report.conversationUpdatedAt = record.lumeyDate(LumeyReportConversationCloudKitSchema.PublicReportField.conversationUpdatedAt)
        report.conversationLastMessageAt = record.lumeyDate(LumeyReportConversationCloudKitSchema.PublicReportField.conversationLastMessageAt)
        if let state = LumeyReportConversationState(rawValue: record.lumeyString(LumeyReportConversationCloudKitSchema.PublicReportField.conversationState)) {
            report.conversationState = state
        }
        try? modelContext.save()
    }

    private func applySnapshot(_ snapshot: LumeyReportConversationSnapshot, to report: SubmittedReport, modelContext: ModelContext) {
    ...
    private func cachedSnapshot(for report: SubmittedReport) -> LumeyReportConversationSnapshot {
        ...
        return LumeyReportConversationSnapshot(
            id: report.conversationRecordName.isEmpty ? report.reportID : report.conversationRecordName,
            reportID: report.reportID,
            sourceAppID: LumeyReportConversationCloudKitSchema.sourceAppID,
    ...
    private func fetchMessages(from root: CKRecord, in database: CKDatabase) async throws -> [LumeyReportConversationMessage] {
        let names = root[LumeyReportConversationCloudKitSchema.ConversationField.messageRecordNames] as? [String] ?? []
        ...
        return ids.compactMap { id in
            ...
            return LumeyReportConversationMessage(
                id: record.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.messageID, fallback: record.recordID.recordName),
                senderRole: LumeyReportConversationSenderRole(rawValue: record.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.senderRole)) ?? .unknown,
                body: record.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.body),
                createdAt: record.lumeyDate(LumeyReportConversationCloudKitSchema.MessageField.createdAt) ?? record.creationDate ?? Date(),
    ...
    private func makeSnapshot(
        from root: CKRecord,
        shareURL: URL,
        messages: [LumeyReportConversationMessage],
        fallbackReport: SubmittedReport
    ) -> LumeyReportConversationSnapshot {
        LumeyReportConversationSnapshot(
            id: root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.conversationID, fallback: root.recordID.recordName),
            reportID: root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.reportID, fallback: fallbackReport.reportID),
            sourceAppID: root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.sourceAppID, fallback: LumeyReportConversationCloudKitSchema.sourceAppID),
            reportType: root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.reportType, fallback: fallbackReport.reportType),
            reportTitle: root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.reportTitle, fallback: fallbackReport.title),
            state: LumeyReportConversationState(rawValue: root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.invitationState)) ?? .notStarted,
            acceptsReplies: (root[LumeyReportConversationCloudKitSchema.ConversationField.acceptsReplies] as? NSNumber)?.boolValue ?? true,
            recordID: root.recordID,
            shareURL: shareURL,
            createdAt: root.lumeyDate(LumeyReportConversationCloudKitSchema.ConversationField.createdAt) ?? root.creationDate,
            updatedAt: root.lumeyDate(LumeyReportConversationCloudKitSchema.ConversationField.updatedAt) ?? root.modificationDate,
            invitedAt: root.lumeyDate(LumeyReportConversationCloudKitSchema.ConversationField.invitedAt),
            acceptedAt: root.lumeyDate(LumeyReportConversationCloudKitSchema.ConversationField.acceptedAt),
            declinedAt: root.lumeyDate(LumeyReportConversationCloudKitSchema.ConversationField.declinedAt),
            reporterUnreadCount: root.lumeyInt(LumeyReportConversationCloudKitSchema.ConversationField.reporterUnreadCount),
            staffUnreadCount: root.lumeyInt(LumeyReportConversationCloudKitSchema.ConversationField.staffUnreadCount),
            messages: messages
        )
    }
    ...
    enum ConversationError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .missingShareMetadata:
                return "Loomey could not load the private conversation invitation."
            case .sharedZoneUnavailable:
                return "The accepted private conversation is not available in Loomey's shared CloudKit database yet."
            case .shareAcceptanceFailed:
                return "Loomey could not prepare the private conversation share."
            case .missingSharedConversation:
