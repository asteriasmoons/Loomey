//
//  LumeyReportConversationNotificationManager.swift
//  Lumey
//
...
enum LumeyReportConversationNotificationManager {
    static let conversationNotificationOpened = Notification.Name("LumeyReportConversationNotificationOpened")
    static let conversationDataDidChange = Notification.Name("LumeyReportConversationDataDidChange")

    private static let container = CKContainer(identifier: LumeyReportConversationCloudKitSchema.containerIdentifier)
    private static let zoneSubscriptionPrefix = "loomey-report-message-events-v3-"
    private static let invitationSubscriptionID = "loomey-report-invitations-v3"
    private static let processedMessagePrefix = "loomey.reportConversation.processedMessage."
    private static let processedInvitationPrefix = "loomey.reportConversation.processedInvitation."
    ...
    static func prepareNotifications() async {
        ...
        if let zones = try? await container.sharedCloudDatabase.allRecordZones() {
            for zone in zones where zone.zoneID.zoneName == LumeyReportConversationCloudKitSchema.zoneName {
                await ensureMessageSubscriptionLogged(database: container.sharedCloudDatabase, zoneID: zone.zoneID)
            }
        }
        let privateZoneID = CKRecordZone.ID(zoneName: LumeyReportConversationCloudKitSchema.zoneName, ownerName: CKCurrentUserDefaultName)
    ...
    static func reportNotifLog(_ message: String) {
        #if DEBUG
        print("[ReportNotif][Loomey] \(message)")
        #endif
    }
    ...
    static func scanForPendingInvitations() async {
        ...
        let predicate = NSPredicate(format: "appID == %@", LumeyReportConversationCloudKitSchema.sourceAppID)
        let query = CKQuery(recordType: LumeyReportConversationCloudKitSchema.RecordType.report, predicate: predicate)
    ...
    static func processInvitation(recordID: CKRecord.ID) async -> Bool {
        ...
        guard let record = try? await database.record(for: recordID),
              record.lumeyString("appID") == LumeyReportConversationCloudKitSchema.sourceAppID,
              record.lumeyString(LumeyReportConversationCloudKitSchema.PublicReportField.conversationState) == LumeyReportConversationState.invited.rawValue else { return false }
        let conversationID = record.lumeyString(LumeyReportConversationCloudKitSchema.PublicReportField.conversationRecordName)
        ...
        let reportID = record.lumeyString("reportID").isEmpty ? recordID.recordName : record.lumeyString("reportID")
        ...
        do {
            try await UNUserNotificationCenter.current().add(UNNotificationRequest(identifier: "loomey-report-conversation-invite-\(conversationID)", content: content, trigger: nil))
            return true
    ...
    private static func ensureInvitationSubscription() async throws {
        ...
        let predicate = NSPredicate(format: "appID == %@", LumeyReportConversationCloudKitSchema.sourceAppID)
        let subscription = CKQuerySubscription(recordType: LumeyReportConversationCloudKitSchema.RecordType.report, predicate: predicate, subscriptionID: invitationSubscriptionID, options: [.firesOnRecordUpdate])
    ...
    private static func ensureMessageSubscription(database: CKDatabase, zoneID: CKRecordZone.ID) async throws {
        ...
        let oldSubscriptionID = "loomey-report-conversations-\(zoneID.ownerName)-\(zoneID.zoneName)"
    ...
    private static func deviceSubscriptionFlagKey(_ subscriptionID: String) -> String {
        "loomey.reportConversation.deviceSubscriptionSaved.\(subscriptionID)"
    }

    private static func notifyForNewStaffMessage(_ message: CKRecord, database: CKDatabase) async -> Bool {
        guard message.recordType == LumeyReportConversationCloudKitSchema.RecordType.message,
              message.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.senderRole) == LumeyReportConversationSenderRole.staff.rawValue else { return false }
        let messageID = message.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.messageID).isEmpty ? message.recordID.recordName : message.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.messageID)
        let processedKey = processedMessagePrefix + messageID
        guard claimNotification(processedKey) else { return false }
        let rootID = (message[LumeyReportConversationCloudKitSchema.MessageField.conversation] as? CKRecord.Reference)?.recordID
            ?? CKRecord.ID(recordName: message.lumeyString(LumeyReportConversationCloudKitSchema.MessageField.conversationRecordName), zoneID: message.recordID.zoneID)
        guard let root = try? await database.record(for: rootID),
              root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.sourceAppID) == LumeyReportConversationCloudKitSchema.sourceAppID,
              root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.invitationState) != LumeyReportConversationState.invited.rawValue else { releaseNotification(processedKey); return false }
        guard await requestAuthorizationIfNeeded() else { releaseNotification(processedKey); return false }
        let reportID = root.lumeyString(LumeyReportConversationCloudKitSchema.ConversationField.reportID)
        let content = UNMutableNotificationContent()
        content.title = "Voxiverse"
        ...
        do {
            try await UNUserNotificationCenter.current().add(UNNotificationRequest(identifier: "loomey-report-conversation-message-\(messageID)", content: content, trigger: nil))
            return true
        } catch {
    ...
    private static func changeTokenKey(for zoneID: CKRecordZone.ID) -> String {
        "loomey.reportConversation.changeToken.\(zoneID.ownerName).\(zoneID.zoneName)"
    }
...
final class LumeyNotificationDelegate: NSObject, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    func application(
        _ application: UIApplication,
        didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil
    ) -> Bool {
        UNUserNotificationCenter.current().delegate = self
        Task { await LumeyReportConversationNotificationManager.prepareNotifications() }
        return true
    }
    ...
    func userNotificationCenter(
        ...
        await MainActor.run {
            NotificationCenter.default.post(
                name: LumeyReportConversationNotificationManager.conversationNotificationOpened,
                object: reportID
            )
        }
    }
    ...
    func application(
        ...
        guard let notification = CKNotification(fromRemoteNotificationDictionary: userInfo) else {
            LumeyReportConversationNotificationManager.reportNotifLog("remote notification received but could not decode CKNotification")
        ...
        Task {
            ...
            if notification.subscriptionID == "loomey-report-invitations-v3",
               let recordID = (notification as? CKQueryNotification)?.recordID {
                notified = await LumeyReportConversationNotificationManager.processInvitation(recordID: recordID)
            } else if let zoneNotification = notification as? CKRecordZoneNotification,
                      let zoneID = zoneNotification.recordZoneID,
                      zoneID.zoneName == LumeyReportConversationCloudKitSchema.zoneName {
                notified = await LumeyReportConversationNotificationManager.processMessageChanges(in: zoneID, databaseScope: zoneNotification.databaseScope)
            } else {
                completionHandler(.noData)
                return
            }
            NotificationCenter.default.post(
                name: LumeyReportConversationNotificationManager.conversationDataDidChange,
                object: nil
            )
