//
//  LumeyReportConversationSchema.swift
//  Lumey
//
...
enum LumeyReportConversationState: String, CaseIterable, Codable, Hashable {
    case notStarted
    case invited
    case accepted
    case declined
...
enum LumeyReportConversationSenderRole: String, Codable, Hashable {
    case staff
    case reporter
    case unknown
}

enum LumeyReportConversationDeliveryState: String, Codable, Hashable {
    case sending
    case sent
    case failed
}

struct LumeyReportConversationMessage: Identifiable, Hashable {
    let id: String
    let senderRole: LumeyReportConversationSenderRole
    let body: String
    let createdAt: Date
    let creatorRecordName: String
    let attachments: [LumeyConversationAttachment]
    var deliveryState: LumeyReportConversationDeliveryState = .sent
...
struct LumeyConversationAttachment: Identifiable, Hashable, Codable {
    let id: UUID
    let name: String
...
struct LumeyReportConversationSnapshot: Identifiable, Hashable {
    let id: String
    let reportID: String
    ...
    let messages: [LumeyReportConversationMessage]

    static func notStarted(for report: SubmittedReport) -> LumeyReportConversationSnapshot {
...
enum LumeyReportConversationCloudKitSchema {
    static let containerIdentifier = "iCloud.im.lystaria.Voxiverse"
    static let sourceAppID = "loomey"
...
extension CKRecord {
    func lumeyString(_ key: CKRecord.FieldKey, fallback: String = "") -> String {
        self[key] as? String ?? fallback
    }

    func lumeyDate(_ key: CKRecord.FieldKey) -> Date? {
        self[key] as? Date
    }

    func lumeyInt(_ key: CKRecord.FieldKey) -> Int {
